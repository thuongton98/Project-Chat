...
function Home(){
    ...
    useEffect(() => {
        ...
        connect.on('join_room',data=>{setroom(data)})
    ...
    if(room!==''){
        ...
        <form onSubmit={(e)=>submitchat(room)}>
          <input ref={ref=>messref=ref} onChange={(e)=>setmess(e.target.value)} type="text" placeholder="type ....." />
          <input  onClick={(e)=>submitchat(room)} className="submit-chat" type="submit" defaultValue="submit" />
        </form>
      </section>
      <footer>
...
if(ok!==''){
   ...
   if(user.length>0){
       ...
       if(find.length<2){
        ...
        {showuseronline(user)}
       {showallmess(allmess)}
        <form onSubmit={(e)=>submitchat('global')}>
          <input ref={ref=>messref=ref} onChange={(e)=>setmess(e.target.value)} type="text" placeholder="type ....." />
          <input  onClick={(e)=>submitchat('global')} className="submit-chat" type="submit" defaultValue="submit" />
        </form>
      </section>
      <footer>
      <div className='copyright'>
