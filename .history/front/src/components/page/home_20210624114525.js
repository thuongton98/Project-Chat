...
function Home(){
    ...
    useEffect(() => {
        ...
        connect.on('allmess',data=>{
            ...
            const findmessroom = x.filter(function(value){
                return (value.room_id[0] === data[0]||value.room_id[0] === data[1])&&(value.room_id[0] === data[0]||value.room_id[0] === data[1])
            })
            console.log(findmessroom)
           setallmess(find)
        })
