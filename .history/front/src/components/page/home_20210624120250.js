...
function Home(){
    ...
    useEffect(() => {
        ...
        connect.on('allmess',data=>{
           ...
           setallmess(find)
           connect.on('join_room',data1=>{
            setroom(data)
            if(x!==undefined){
                const findmessroom = data.filter(function(value){
                    return (value.room_id[0] === data1[0]||value.room_id[0] === data1[1])&&(value.room_id[0] === data1[0]||value.room_id[0] === data1[1])
                })
                setmessroom(findmessroom)
            }
